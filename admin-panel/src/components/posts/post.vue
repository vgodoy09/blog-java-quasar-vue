<template>
  <div class="post">
    ...
    <div class="post_header mt-4">
      ...
      <div v-if="typeof post.author_id !== 'undefined'" class="btn-group float-right" role="group" aria-label="Basic example">
        <router-link
      ...
      <div class="tags">
        <span>tags:</span>
        <ul v-if="typeof post.listTags !== 'undefined' && post.listTags.length !== 0">
          <li v-for="tag in post.listTags" :key="tag.id">
    ...
    <p class="lead post_info">
      by
      <template v-if="typeof post.author_id !== 'undefined'">
        <router-link :to="'/users/' + post.author_id.slug + '/posts'">{{ post.author_id.username }}</router-link>
      </template>
      <template v-else>
        unknown
      </template>
...
export default {
  ...
  created: function () {
    ...
    let data = {
      ...
      userLogado: this.$auth.user().id
    }

    this.$store.dispatch('comments/getPostComments', data);
  },
